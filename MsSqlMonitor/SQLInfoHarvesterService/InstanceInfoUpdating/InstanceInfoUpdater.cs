...
namespace SQLInfoCollectionService.InstanceInfoUpdating
{
    ...
    public class InstanceInfoUpdater
    {
        ...
        public async Task<InstanceInfo> UpdateAsync(int instanceID, IInstanceDataCollector collector)
        {
            // Get information about databases and their principals (inluding permissions).
            // Stars first because it has more data and more time to process it.
            //DataTable databases =  collector.GetDatabases();
            DataTable databases = await collector.GetDatabasesAsync(); // List of all databases on the instance with some basic information (size, date of creation).
            ...
            // Get information about instance and it's principals.
            var instanceDetailsTask = await collector.GetInstanceInfoAsync();
            var instanceRolesTask = await collector.GetInstanceRolesAsync();
            var instanceLoginsTask = await collector.GetInstanceLoginsAsync();
            var instancePermissionsTask = await collector.GetInstancePermissionsAsync();
            ...
            InstanceInfo result = ParseInstancePrincipals(
                instanceID,
                ( instanceDetailsTask).Rows[0],
                 instanceRolesTask,
                 instanceLoginsTask,
                 instancePermissionsTask);
        ...
        private InstanceInfo ParseInstanceInfo(InstanceInfo instanceInfo, DataRow instanceDetails)
        {
            ...
            instanceInfo.Version = instanceDetails[3].ToString();
            return instanceInfo;
        }
